...
public class AccountApiExceptionHandler {
    ...
    public AccountApiErrorResponse handleConstraintViolation(ConstraintViolationException e, ServletWebRequest request) {
        return AccountApiErrorResponse.builder()
                .timestamp(now().toEpochMilli())
                .path(request.getRequest().getRequestURI())
                .status(BAD_REQUEST.value())
                .error(BAD_REQUEST.getReasonPhrase())
                .message(e.getMessage())
                .errors(e.getConstraintViolations().stream().map(cv -> AccountApiErrorResponse.AccountApiError.builder()
