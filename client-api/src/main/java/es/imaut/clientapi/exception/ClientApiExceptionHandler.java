...
public class ClientApiExceptionHandler {
    ...
    public ClientApiErrorResponse handleConstraintViolation(ConstraintViolationException e, ServletWebRequest request) {
        return ClientApiErrorResponse.builder()
                .timestamp(now().toEpochMilli())
                .path(request.getRequest().getRequestURI())
                .status(BAD_REQUEST.value())
                .error(BAD_REQUEST.getReasonPhrase())
                .message(e.getMessage())
                .errors(e.getConstraintViolations().stream().map(cv -> ClientApiErrorResponse.ClientApiError.builder()
